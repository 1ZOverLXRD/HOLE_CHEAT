...
namespace OSImGui
{
    ...
    void OSImGui_External::AttachAnotherWindow(DWORD PID, std::function<void()> CallBack)
    {
        ...
        if (hWnd == NULL)
            throw OSException("Window with PID not found");
        Window.Name = "KENROOB";
        Window.wName = StringToWstring(Window.Name);
        Window.ClassName = "ISCRAZY";
        Window.wClassName = StringToWstring(Window.ClassName);
        Window.BgColor = ImColor(0, 0, 0, 0);
        Window.Transparent = true;
        ...
        if (GetWindowTextA(hWnd, title, sizeof(title)) == 0)strcpy_s(title, ""); 
        if (GetClassNameA(hWnd, className, sizeof(className)) == 0)strcpy_s(className, "");

        DestWindow.hWnd = hWnd;
        DestWindow.Name = title;
        DestWindow.ClassName = className;
    ...
    void OSImGui_External::AttachAnotherWindow(std::string processName, std::function<void()> CallBack)
    {
        if (!CallBack)
            throw OSException("CallBack is empty");
        ...
        if (pid == 0)
            throw OSException("Process not found");

        HWND hWnd = FindWindowByPID(pid);
        if (hWnd == NULL)
            throw OSException("Window for process not found");
        Window.Name = "KENROOB";
        Window.wName = StringToWstring(Window.Name);
        Window.ClassName = "ISCRAZY";
        Window.wClassName = StringToWstring(Window.ClassName);
